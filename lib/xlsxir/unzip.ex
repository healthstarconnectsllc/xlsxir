defmodule Xlsxir.Unzip do
  ...
  defp extract_from_zip(path, file_list, :memory),
    do: :zip.extract(to_charlist(path), [{:file_list, file_list |> Enum.map(&to_charlist/1)}, :memory])

  defp extract_from_zip(path, file_list, {:file, dest_path}),
    do: :zip.extract(to_charlist(path), [{:file_list, file_list |> Enum.map(&to_charlist/1)}, {:cwd, to_charlist(dest_path)}])
